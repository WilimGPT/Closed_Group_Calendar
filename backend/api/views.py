...
def update_course(request, language, courseId):
    ...
    course = next((c for c in teacher["bookings"] if c["id"] == courseId), None)
    if not course:
        return JsonResponse({"error": "Course not found"}, status=404)

    # Update fields, keep ID unchanged
    course["groupReference"] = payload["groupReference"]
    course["timeZone"] = payload["timeZone"]
    course["durationMinutes"] = payload["durationMinutes"]
    course["sessions"] = payload["sessions"]
